...
import org.fog.placement.ModulePlacementAllModuleForEachFogDevice;
...
public class TestConsum {
    ...
    private static int NUM_DEV_PER_SW = 1;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            broker.submitApplication(application, 0,
                    new ModulePlacementAllModuleForEachFogDevice(fogDevices, sensors, actuators, application));
    ...
    private static Application createApplication(String appId, int userId){
         ...
         */
        application.addAppModule("A", 1000, 100);
        application.addAppModule("B", 1000, 100);
        application.addAppModule("C", 1000, 100);
        application.addAppModule("D", 1000, 100);
        application.addAppModule("E", 1000, 100);

        /*
         * Connecting the application modules (vertices) in the application model (directed graph) with edges
         */
        application.addAppEdge("SENSED_DATA", "A", 30000, 10*1024, "SENSED_DATA", Tuple.UP, AppEdge.SENSOR);
        application.addAppEdge("E", "ACTION", 1000, 1*1024, "ACTION", Tuple.DOWN, AppEdge.ACTUATOR);  // adding edge from Client module to Display (actuator) carrying tuples of type SELF_STATE_UPDATE

        application.addAppEdge("A", "B", 20000, 10 * 1024, "ab", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("B", "C", 20000, 10 * 1024, "bc", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("B", "D", 20000, 10 * 1024, "bd", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("C", "E", 20000, 10 * 1024, "ce", Tuple.UP, AppEdge.MODULE);
        application.addAppEdge("D", "E", 20000, 10 * 1024, "de", Tuple.UP, AppEdge.MODULE);

        /*
         * Defining the input-output relationships (represented by selectivity) of the application modules.
         */
        application.addTupleMapping("A", "SENSED_DATA", "ab", new FractionalSelectivity(1.0));
        application.addTupleMapping("B", "ab", "bc", new FractionalSelectivity(0.5));
        application.addTupleMapping("B", "ab", "bd", new FractionalSelectivity(0.5));
        application.addTupleMapping("C", "bc", "ce", new FractionalSelectivity(1));
        application.addTupleMapping("D", "bd", "de", new FractionalSelectivity(1));
        application.addTupleMapping("E", "de", "ACTION", new FractionalSelectivity(1));
        application.addTupleMapping("E", "ce", "ACTION", new FractionalSelectivity(1));



        final AppLoop loop1 = new AppLoop(new ArrayList<String>(){{add("SENSED_DATA");add("A");add("B");add("C");add("E");add("ACTION");}});
    ...
    private static void createPhysicalTopology(int userId, String appId, Application application) {
        ...
        Switch dcSw = new Switch("CLOUD_SW");
...
//            PhysicalTopology.getInstance().addLink(esw.getId(), sw.getId(), 5, 1000);
            ...
            for (int j = 0; j < NUM_DEV_PER_SW ; j++) {
                String suffix = i+"-"+j;
                ...
                PhysicalTopology.getInstance().addLink(sw.getId(), endDevice.getId(), 10, 1000);
            }
